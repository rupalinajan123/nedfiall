<?php /*Template Name: Blogs and Articles */ 
?>
<?php 
get_header();
$page_data = get_page_by_path( 'blogs-articles-list' );
...
<form method="post" action="<?php echo site_url()?>/blogs-articles-list/" id="form_id">
            ...
            <div class="row">
                <div class="col-12 col-lg-4 nf-sidebar-c-width">
                    ...
                    <div class="courses-details-sidebar-area search-sidebar nf-sidebar nf-kyal">
                              ...
                              <a class="btn-link nf-color-2" data-toggle="collapse" href="#department-filter">
                                ...
                                <div class="widget-link collapse show" id="state-filter">
                                    <ul class="sidebar-link nf-sidebar-scroll">
                                        ...
                                        foreach($var['choices'] as $choice)
                                        {
                                            if($_POST['topicx']==$choice) $checked = 'checked'; 
                                            else if(is_array($_POST['topicx']) && in_array($choice,$_POST['topicx'])) $checked = 'checked'; 
                                            else  $checked = '';
                                            echo '
                                            <li>
                                            <div class="nf-checkbox-wrap">
                                            <input class="check_topic" value="'.$choice.'" type="checkbox" '.$checked.' id="topic_'.$k.'" name="topicx" class="check_topic">
                                            <label for="topic_'.$k.'">'.$choice.'</label>
                                            </div>
                                            </li>
                                            ';
                                            $k++;
                        ...
                        if(!empty($_POST['topicx']))
                        {
                            if(!is_array($_POST['topicx'])) $_POST['topicx']=[$_POST['topicx']];
                        ...
                        <div class="col-12 col-lg-8 nf-listing-c-width">
                           ...
                           <?php if(!empty($_POST['state']) or !empty($_POST['topicx']) or !empty($_POST['sectors'])){?>
                              ...
                              <?php }?>
                              <?php if(!empty($_POST['topicx'])){?>
                               <div class="col-12 col-lg-4">
                                <a href="#">
                                  <img src="<?php echo get_template_directory_uri(). '/assets/'?>img/study-in-north/department.png" class="nf-w-t2">
                                  <span><?php if(is_array($_POST['topicx'])) echo Implode(',<br>',$_POST['topicx']);else echo $_POST['topicx'];?></span>
                              </a>
                          </div>
                      <?php }?>
...
<div class="nf-know-listing-block mt-4">
    <div class="row">
        ...
        while($blog_posts->have_posts()) { 
            ...
            <div class="gallery_product col-lg-4 col-md-6 col-sm-6 col-xs-12 filter Recent">
              <?php if ($flag=="In-House") { ?>
                <input type="hidden" id="copylink<?php echo $record?>" value="<?php echo get_permalink( $post->ID ); ?>">
                <?php } else{ ?>
                  <input type="hidden" id="copylink<?php echo $record?>" value="<?php echo $blog_link; ?>">
                  <?php } ?>
                <div class="nf-blog-list">
                  <div class="img-wrap">
     ...
     <?php/*?> <div class="gallery_product col-lg-4 col-md-6 col-sm-6 col-xs-12 filter Recent">
        <div class="nf-blog-list">
            <div class="infoGallery">
                ...
                <?php if ($flag=='In-House') { ?>
                        ...
                        <h4><?php echo $blog_description?>.. <a style="cursor: pointer; color: #1673e7;" href="<?php echo get_permalink( $post->ID )?>" >Read More</a></h4><br>
                   
                <?php } ?>

                <?php if ($flag=='External') { ?>

                    <h4><?php echo $blog_description?>.. <a style="cursor: pointer; color: #1673e7;" href="<?php echo $blog_link;?>" target="_blank">Read More</a></h4><br>
...
$(document).ready(function(){
    $('.check_topic').click(function() {
        $('.check_topic').not(this).prop('checked', false);
        document.getElementById("form_id").submit();
    });
});
