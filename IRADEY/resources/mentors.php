...
if(!empty($sector)) $_POST['sectors']=$sector;
if($sector=='Livelihood and Planning') $_POST['state']='Assam';
if($sector=='Business Administration') $_POST['state']='Manipur';

get_header(); 
$page_data = get_page_by_path( 'mentors' );
// $type = get_field_object('field_60d2d5f827be0');

//  if ($_POST['mentorsAndIncubators'] == 'incubators') { 
//     echo "hii this is incubator page"; exit; 
...
<div class="bannerbg nf-gradient_5 justify-content-start pt-3 nf-height-100">
...
<form method="post" action="<?php echo site_url()?>/mentors" id="form_id">
...
<div class="widget mb-20 widget-padding white-bg">
    ...
    <?php
                      ...
                      <img src="<?php echo get_template_directory_uri(). '/assets/'?>img/study-in-north/state-white.png" alt="state-white"> <span> State (<?php echo count($return_state);?>) </span></a>
                      <div class="widget-link collapse show" id="state-filter">
                        ...
                        ?>
                            ...
                            if(!empty($_POST['state']) )
                              ...
                              if(!empty($_POST['sectors']) && !in_array($_POST['sectors'], $return_sectors) && $sector=='') $_POST['sectors']='';
                              wp_reset_query();
                              
                            //=======ajax end
                            ?>
                <?php $var = get_field_object('field_60d2d5ba27bdf');?>
                ...
                <div class="widget-link collapse show" id="state-filter">
                  ...
                  foreach($return_sectors as $choice)
                  {
                    ...
                    <li>
                      <div class="nf-checkbox-wrap">
                        <input class="check_sectors" value="'.$choice.'" type="checkbox" '.$checked.' id="dept_'.$k.'" name="sectors" onclick="javascript:this.form.submit()">
...
if(!empty($_POST['sectors']))
  ...
  if($_POST['keyword']!='')
  {
    $keyw = array(
        ...
        array(
            'key'     =>  'area_of_specialization',
                  'value'    =>  $_POST['keyword'],
                  'compare'  => 'LIKE'
        ),
        array(
            'key'     =>  'associated_institute',
                  'value'    =>  $_POST['keyword'],
                  'compare'  => 'LIKE'
        ),
        array(
    ...
    <?php
      $record=0;
                            while($blog_posts->have_posts()) {
                                ...
                                if($values)
                                {
                                    ...
                                    foreach($values as $key => $value)
                                    {
                                        ...
                                        if($key=='mentor_name'){ $mentor_name = $value;  }
                                        if($key=='type'){ $type = $value;  }

                                        if($key=='area_of_specialization'){ $area_of_specialization = $value; }
                                        if($key=='associated_institute'){ $associated_institute = $value;  }
                                        if($key=='contact_details1'){ $contact_details1 = $value;  }
            ...
            /*if(((isset($_POST) && 
              ...
              && ($_POST['area_of_specialization']==$area_of_specialization or $_POST['area_of_specialization']=='' or (is_array($_POST['area_of_specialization']) && in_array($area_of_specialization,$_POST['area_of_specialization'])))
              

               && (($_POST['keyword']!='' && (strpos($state, $_POST['keyword']) !== false or strpos($sectors, $_POST['keyword']) !== false)) or $_POST['keyword']=='')

            ) or !isset($_POST) && $type='mentor')){ */
    ...
    <div class="nf-know-listing-block">
        <div class="mentor-table">
            <div class="nameHeading">
                <div class="textIcon">MN</div>
                <span>Mentor Name</span>
                <h3><?php echo $mentor_name; ?></h3>
            </div>
            <div class="infoNewAlltabs">
                <ul>
                    <li>Area Of Specialization</li>
                    <li>Associated Institute/Incubator</li>
                    <li><strong><?php echo $area_of_specialization; ?></strong></li>
                    <li><strong><?php echo $associated_institute; ?></strong></li>
                </ul>
            </div>
            <div class="contectTrInfo2 borderTop">
                <ul>
                    <li class="callIcon">Contact Details
                        <span><?php echo $contact_details1; ?></span>
                    </li>
                    <li class="mailIcon">Email Id
                        <span><a href="#"><span><?php echo $email_id; ?></span></a></span>
                    </li>
                </ul>
            </div>
        </div> </br>
