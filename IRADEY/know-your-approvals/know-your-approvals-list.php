  ...
  //$paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1;
      ...
      ?>
          ...
          <?php }?>
           ...
           <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist" style="justify-content:left">
  <li class="nav-item" role="presentation">
    <a class="nav-link active" id="pills-home-tab" data-toggle="pill" href="#nf-knowlist-tab13" role="tab">Pre-Operation</a>
  </li>
  <li class="nav-item" role="presentation">
    <a class="nav-link" id="pills-profile-tab" data-toggle="pill" href="#nf-knowlist-tab23" role="tab">Post-Operation</a>
  </li>
</ul>
...
<div class="tab-content" id="pills-tabContent">
  ...
  <!-- Pre-Operation Tab -->
  <div class="tab-pane fade show active" id="nf-knowlist-tab13" role="tabpanel">
    <div class="nf-k-list-table">
      <div class="table-responsive">
        <table>
  ...
  <!-- Post-Operation Tab -->
  <!-- <div class="tab-pane fade" id="nf-knowlist-tab23" role="tabpanel">
    <div class="nf-k-list-table">
      <div class="table-responsive">
        <table>
          <thead class="nf-thead-color-3">
          ...
          <tbody>
          <?php
            // You should fetch post-operation data here
            $k=0;
            $poperation = get_post_meta( $post->ID, '_event_operation_post_value_key', true ); 
            $pnature = get_post_meta( $post->ID, '_event_onature_post_value_key', true );
            $pdescription = get_post_meta( $post->ID, '_event_odescription_post_value_key', true ); 
            $pissue = get_post_meta( $post->ID, '_event_oissue_post_value_key', true ); 
            $plink = get_post_meta( $post->ID, '_event_olink_post_value_key', true );
            $pentity = get_post_meta( $post->ID, '_event_oentity_post_value_key', true ); 

            if(!empty($poperation)) {
              $poperation = explode('*****',$poperation);
              $pnature = explode('*****',$pnature);
              $pdescription = explode('*****',$pdescription);
              $pissue = explode('*****',$pissue);
              $plink = explode('*****',$plink);
              $pentity = explode('*****',$pentity);

              for($i=0;$i<count($poperation);$i++) {
                $k++;
                ?>
                <tr>
                  <td><?php echo $poperation[$i]?></td>
                  <td><?php echo $pnature[$i]?></td>
                  <td><?php echo $pdescription[$i]?></td>                      
                  <td><?php echo $pissue[$i]?></td>
                  <td><a href="<?php echo $plink[$i]?>" target="_blank"><?php echo $plink[$i]?></a></td>
                </tr>
              <?php }
            } 
            if($k==0) { echo '<tr><td colspan="5">No Record Found</td></tr>'; }
