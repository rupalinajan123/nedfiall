        ...
        <div class="col-lg-8 d-flex align-items-center">
            <div class="nf-data">
                ...
                <div class="nf-jobPortal-btn">
                    <a href="<?php echo site_url()?>/ncs-job-home/" target="_blank" class="btn" id="vacancies-link">Vacancies at NCS Portal</a>
        ...
        <div class="swiper mySwiper">
            <div class="swiper-wrapper">  
                ...
                while ($testimonial_query->have_posts()) : $testimonial_query->the_post();
                    ...
                    // print_r($values_testimonial);
                    $testimonial_image = wp_get_attachment_image_src(get_post_thumbnail_id($testimonial_query->ID), "original");
                    $youtube_link = $values_testimonial['video_link']; // Assuming 'video_link' is the field name
                    
                    // print_r($youtube_link);
                    // Handling YouTube Shorts or Standard YouTube Links
                        if (strpos($youtube_link, 'youtube.com/shorts') !== false) {
                            // Convert Shorts link to embed format
                            preg_match('/shorts\/([a-zA-Z0-9_-]+)/', $youtube_link, $matches);
                    ...
                    <div class="testomonial-img">
                        <div class="vid-slider">
                            <div class="vid-wrapper">
                                <div class="vid item">
                                    <iframe width="0" height="0" src="<?php echo esc_url($youtube_link); ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                    <?php if ($testimonial_image): ?>
                                        <img src="<?php echo esc_url($testimonial_image[0]); ?>" class="img-fluid">
                                    <?php endif; ?>
...
// Fetch all necessary fields including 'rating'
          ...
          foreach ($results as $row) {
            if (!isset($clientStories[$row->lead_id])) {
              $clientStories[$row->lead_id] = [
                'value' => '',
                'name' => '',
                'rating' => 0,  // Add default rating field
              ];
            }
            ...
            } elseif ($row->name == 'rating') {  // Assuming 'rating' is the field in the database
              $clientStories[$row->lead_id]['rating'] = (int)$row->value;  // Convert to integer
            }
          }

          // Display client stories
          foreach ($clientStories as $story) {
            $rating = $story['rating'];  // Fetch the rating value
        ?>
        <div class="swiper-slide">
          <div class="client-info">
            <div class="client-info-main">
              ...
              <div class="c-rating">
                <?php
                  // Loop through to display filled and empty stars
                  for ($i = 1; $i <= 5; $i++) {
                    if ($i <= $rating) {
...
jQuery(document).ready(function($) {
    // When the "Vacancies at NCS Portal" link is clicked
    $('#vacancies-link').on('click', function(e) {
        e.preventDefault(); // Prevent the default action of the link
        var link = $(this).attr('href'); // Get the URL to navigate to

        // Make an AJAX call to update the hit count
        $.ajax({
            url: '<?php echo admin_url("admin-ajax.php"); ?>',
            type: 'POST',
            data: {
                action: 'ncs_portal_hit_count'
            },
            success: function(response) {
                if (response.success) {
                    console.log('Hit count updated:', response.data);
                }
                // Redirect to the link after the AJAX call is successful
